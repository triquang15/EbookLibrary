...
export const Loans = () => {
    ...
    useEffect(() => {
        const fetchUserCurrentLoans = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `http://localhost:8080/api/books/checkout/loans`;
    ...
    async function returnBook(bookId: number) {
        const url = `http://localhost:8080/api/books/checkout/return/?bookId=${bookId}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    async function renewLoan(bookId: number) {
        const url = `http://localhost:8080/api/books/checkout/renew/loan/?bookId=${bookId}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
